  ...
  useEffect(() => {
    if (property) {
      ...
      setFormData((prev: any) => ({
        property_code: prev.property_code || `RKS-${Math.floor(10000 + Math.random() * 90000)}`,
        project_id: prev.project_id || projects[0]?.id || '',
        location_id: prev.location_id || locations[0]?.id || '',
        property_type: prev.property_type || 'Residential Plot',
        category: prev.category || 'Standard',
        status: prev.status || 'AVAILABLE',
        plot_number: prev.plot_number || '',
        unit_number: prev.unit_number || '',
        block: prev.block || '',
        floor: prev.floor || '',
        survey_number: prev.survey_number || '',
        approval_number: prev.approval_number || '',
        area_sqft: prev.area_sqft || 2400,
        rate_per_sqft: prev.rate_per_sqft || 5200,
        total_price: prev.total_price || (2400 * 5200),
        negotiable: prev.negotiable || false,
        minimum_price: prev.minimum_price || '',
        registration_charges: prev.registration_charges || '',
        other_charges: prev.other_charges || '',
        facing: prev.facing || 'East',
        road_width: prev.road_width || '40 ft',
        bedrooms: prev.bedrooms || 0,
        bathrooms: prev.bathrooms || 0,
        ownership: prev.ownership || 'Freehold',
        broker: prev.broker || '',
        assigned_to: prev.assigned_to || '',
        description: prev.description || '',
        description_ta: prev.description_ta || '',
        internal_notes: prev.internal_notes || '',
        latitude: prev.latitude || '',
        longitude: prev.longitude || '',
        amenities: prev.amenities || ['24x7 Water', 'Gated Security'],
      }));
    }
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6">
            ...
            {formTab === 'specs' && (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-300">Latitude (Optional)</label>
                  <input
                    type="number"
                    step="any"
                ...
                <div>
                  <label className="text-xs font-semibold text-zinc-300">Longitude (Optional)</label>
                  <input
                    type="number"
                    step="any"
